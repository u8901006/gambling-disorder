...
const SYSTEM_PROMPT = `你是賭博疾患（Gambling Disorder）領域的資深研究員與科學傳播者。你的任務是：
...
const ALLOWED_TAGS = [
  "賭博疾患", "病態性賭博", "問題性賭博", "行為成癮", "神經科學",
  "神經影像", "fMRI", "多巴胺", "獎賞迴路", "衝動控制",
  "認知行為治療", "CBT", "動機式訪談", "藥物治療", "心理治療",
  "Naltrexone", "SSRI", "流行病學", "盛行率", "篩檢工具",
  "PGSI", "SOGS", "自殺風險", "自殺防治", "憂鬱症",
  "焦慮症", "物質使用", "酒精使用", "ADHD", "雙相情緒障礙",
  "青少年", "大學生", "老年人", "運動員", "線上賭博",
  "手機下注", "運動博彩", " Loot box", "電競博彩", "密碼貨幣賭博",
  "公共政策", "法規", "傷害減少", "責任賭博", "社會決定因素",
  "家庭傷害", "債務", "犯罪", "汙名", "飲食", "營養",
  "睡眠", "身體活動", "運動介入", "人工智慧", "數位介入",
  "自我排除", "復發預防", "正念", "接受與承諾療法", "體驗性迴避",
];
...
async function loadPapers(path) {
...
function buildUserPrompt(papersData) {
  ...
  return `以下是 ${dateStr} 從 PubMed 抓取的最新賭博疾患（Gambling Disorder）相關文獻（共 ${count} 篇）。
...
每篇 paper 的 tags 請從以下選擇：${ALLOWED_TAGS.join("、")}
...
function tryParseJson(text) {
  ...
  const fixedBraces = repairedTruncatedJson(cleaned);
  try {
    return JSON.parse(fixedBraces);
  } catch {}
  return null;
}

function repairedTruncatedJson(str) {
  let s = str;
  let braceCount = 0;
  let bracketCount = 0;
  let inStr = false;
  ...
  for (const model of MODELS) {
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (!parsed) {
          console.error(`[WARN] JSON parse failed for ${model} attempt ${attempt}`);
          if (attempt < MAX_RETRIES) await new Promise((r) => setTimeout(r, 5000));
          continue;
        }

        console.error(`[INFO] Analysis complete with ${model}: ${parsed.top_picks?.length ?? 0} top picks, ${parsed.all_papers?.length ?? 0} total`);
        return parsed;
      } catch (err) {
        if (err.name === "TimeoutError") {
          console.error(`[WARN] ${model} timed out on attempt ${attempt}`);
        } else {
...
<title>Gambling Disorder Research &middot; 賭博疾患研究文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 賭博疾患研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ext-link { display: flex; align-items: center; gap: 14px; padding: 18px 24px; background: var(--card-bg); border: 1px solid var(--line); border-radius: 24px; text-decoration: none; color: var(--text); transition: all 0.2s; box-shadow: 0 8px 30px rgba(61,36,15,0.04); }
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Gambling Disorder Research &middot; 賭博疾患研究文獻日報</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/gambling-disorder">GitHub</a></span>
  </footer>
</div>
</body>
</html>`;
...
async function updateSummarizedPmids(papersData) {
  ...
  const set = new Set([...existing, ...papersData.papers.map((p) => p.pmid).filter(Boolean)]);
  await writeFile(path, JSON.stringify([...set].sort(), null, 2), "utf-8");
  console.error(`[INFO] Updated summarized_pmids.json (${set.size} total)`);
}
...
async function main() {
  ...
  if (!papersData.papers?.length) {
    ...
    console.error(`[INFO] Empty report saved to ${output}`);
    return;
  }
  ...
  analysis.date = analysis.date || papersData.date || taipeiDate();
  ...
  await updateSummarizedPmids(papersData);
}
